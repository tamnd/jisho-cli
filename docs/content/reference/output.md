...
describes what users get. Pick a format with `-o`, or let jisho choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jisho <command> -o table   # aligned columns for reading
jisho <command> -o jsonl   # one JSON object per line, for piping
jisho <command> -o json    # a single JSON array
jisho <command> -o csv     # spreadsheet friendly
jisho <command> -o tsv     # tab-separated
jisho <command> -o url     # just the URL column
jisho <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jisho <command> --fields id,title,url
```
...
```bash
jisho <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jisho <command>            # a table, because this is a terminal
jisho <command> | wc -l    # JSONL, because this is a pipe
```
